...
int main_process(uint8_t read, uint8_t write, uint8_t output_file_named, uint8_t input_file_named, char *output_file_name, char *input_file_name);
...
int main_process(
    uint8_t c_read,
    uint8_t c_write,
    uint8_t c_output_file_named,
    uint8_t c_input_file_named,
    ...
    char *c_input_file_name)
{
    ...
    {
        printf("\r\nFailed to get number of devices (error : %d)\r\n", ftdi_status);
        return -1;
    }
    ...
    if (ftdi_status == FT_OK)
    {
        ...
        {
            printf("\r\nDevice Index: %d ", i);
            printf("\r\nFlags: %x ", ftdi_device_list[i].Flags);
            printf("\r\nType: %d ", ftdi_device_list[i].Type);
            printf("\r\nID: %x ", ftdi_device_list[i].ID);
            printf("\r\nLocation ID: %x ", ftdi_device_list[i].LocId);
